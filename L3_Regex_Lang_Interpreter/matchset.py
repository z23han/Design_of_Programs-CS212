...
def matchset(pattern, text):
    ...
    if 'lit' == op:
        # remove x and return rest of the text
        return set([text[len(x):]]) if text.startswith(x) else null
    elif 'seq' == op:
        # since it is a sequence, find t1 for x pattern inside x, and find t2 for y pattern inside t1
        return set(t2 for t1 in matchset(x, text) for t2 in matchset(y, t1))
    elif 'alt' == op:
        # find the companion covering x and y patterns
        return matchset(x, text) | matchset(y, text)
    elif 'dot' == op:
        # pinpoint at the case of any character
        return set([text[1:]]) if text else null
    elif 'oneof' == op:
        # look at any of the character out of the pattern
        retText = ''
        if x in text:
            xIndex = text.index(x)
            retText += text[:xIndex] + text[xIndex+1:]
            return set([retText])
        else:
            return null
    elif 'eol' == op:
