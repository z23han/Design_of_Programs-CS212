...
def subway(**lines):
    ...
    ## your code here
    subway_dict = {}
    for lineName, stationsStr in lines.iteritems():
        stations = stationsStr.split()
        for i in range(len(stations)):
            if stations[i] not in subway_dict:
                subway_dict[stations[i]] = {}
            if i == 0:
                subway_dict[stations[i]][stations[i+1]] = lineName
...
def ride(here, there, system=boston):
    ...
    ## your code here
    assert here in system, "here not in the system !"
    assert there in system, "there not in the system !"
    if there in system[here]:
        return [here, there]
    verticesPool = system.keys()
    flag = {}
    prev = {}
    found = 0
    for st in verticesPool:
        flag[st] = False
        prev[st] = -1
    queue = []
    queue.append(here)
    while queue != []:
        st = queue.pop(0)
        for neighbor in system[st].keys():
...
def longest_ride(system):
    ...
    ## your code here
    # I'm gonna try DFS here, even though I know it won't work
...
def test_ride():
    ...
    print ride('mattapan', 'foresthills')
    print ride('newton', 'alewife')
